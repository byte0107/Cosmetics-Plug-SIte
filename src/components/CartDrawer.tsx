...
const DELIVERY_RATES = { NUL: 0, Roma: 10, Other: 50 };

export default function CartDrawer() {
  const {
    isCartOpen, closeCart, cart, currency,
    removeFromCart, updateQuantity, getCartTotal, clearCart
  } = useStore();
  ...
  const getWhatsAppLink = () => {
    ...
    const text = `Lumelang Cosmetic Plug! 🌟%0A%0AOrder:%0A${itemsText}%0A%0ADelivery: ${deliveryZone}%0ATotal: ${displayPrice(total)}%0A%0APlease confirm!`;
  ...
  return (
    <>
      ...
      {/* Drawer */}
      <div className="fixed inset-x-0 bottom-0 top-12 md:top-0 md:left-auto md:right-0 md:w-[480px] bg-zinc-50 rounded-t-[32px] md:rounded-none flex flex-col z-[100] shadow-2xl">

        {/* Header */}
        <div className="bg-white px-6 pt-6 pb-4 border-b border-zinc-100 rounded-t-[32px] md:rounded-none shrink-0">
          <div className="flex items-center justify-between mb-6">
            <button
              onClick={() => step > 1 ? setStep((step - 1) as CheckoutStep) : closeCart()}
              className="active:scale-95 transition-all w-10 h-10 flex items-center justify-center -ml-2"
            >
        ...
        <div className="flex-1 overflow-y-auto hide-scrollbar p-6 pb-32">
          ...
          {cart.length === 0 && step === 1 ? (
            <div className="h-full flex flex-col items-center justify-center text-center">
              ...
              <p className="text-zinc-500 text-sm mb-8">Add some beauty to your life!</p>
              <button
                onClick={closeCart}
            ...
            <>
                ...
                <div>
                  ...
                  <div className="space-y-3 mb-8">
                    {(['NUL', 'Roma', 'Other'] as DeliveryZone[]).map(zone => (
                      <button key={zone} onClick={() => setDeliveryZone(zone)}
                        className={clsx(
                          'w-full flex items-center justify-between p-5 rounded-[28px] border-2 transition-all active:scale-[0.98]',
                          deliveryZone === zone ? 'border-primary bg-primary/5' : 'border-zinc-100 bg-white hover:border-zinc-200'
                        )}>
                        <div className="flex items-center gap-3">
                          <div className={clsx('w-5 h-5 rounded-full border-2 flex items-center justify-center', deliveryZone === zone ? 'border-primary' : 'border-zinc-300')}>
                            {deliveryZone === zone && <div className="w-2.5 h-2.5 bg-primary rounded-full" />}
                          </div>
                          <span className="font-bold text-zinc-900">
                            {zone === 'NUL' ? 'NUL Campus' : zone === 'Roma' ? 'Roma Area' : 'Other Areas'}
                          </span>
                        </div>
                  ...
                  <div className="bg-zinc-900 rounded-[44px] p-8 relative overflow-hidden mb-8 shadow-xl">
                    <div className="absolute -right-8 -top-8 w-36 h-36 bg-primary/25 rounded-full blur-3xl" />
                    <div className="relative z-10">
                      <p className="text-zinc-400 text-xs mb-6">Pay to one of these numbers, then confirm via WhatsApp.</p>
                      <div className="space-y-4">
                        <div onClick={() => handleCopy('50963071', 'mpesa')}
                          ...
                          <span className="material-symbols-outlined text-white">{copied === 'mpesa' ? 'check_circle' : 'content_copy'}</span>
                        </div>
                        <div onClick={() => handleCopy('62495282', 'ecocash')}
                          className="bg-white/10 hover:bg-white/20 transition-colors p-4 rounded-[24px] flex items-center justify-between cursor-pointer active:scale-95">
                          <div>
                          ...
                          <span className="material-symbols-outlined text-white">{copied === 'ecocash' ? 'check_circle' : 'content_copy'}</span>
                        </div>
                      </div>
                    </div>
                  </div>
              ...
              {step === 3 && (
                <div className="flex flex-col items-center justify-center text-center pt-10">
                  ...
                  <p className="text-zinc-500 text-sm mb-8 px-4">Complete these 2 steps to get your glow.</p>
                  <div className="w-full bg-white rounded-[36px] p-6 shadow-sm border border-zinc-50 text-left">
                    <div className="flex gap-4 mb-6">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-zinc-500 leading-relaxed">Pay <strong className="text-primary">{displayPrice(total)}</strong> to M-Pesa or EcoCash from step 2.</p>
                      </div>
                    </div>
                    <div className="flex gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-zinc-500 leading-relaxed">Tap below to send your order details and proof of payment.</p>
                      </div>
                    </div>
                  </div>
                </div>
        ...
        {/* Fixed bottom CTA */}
        {cart.length > 0 && (
          <div className="absolute bottom-0 w-full bg-white/90 backdrop-blur-2xl border-t border-zinc-50 px-6 py-5 shadow-[0_-12px_40px_rgba(0,0,0,0.07)]">
            {step === 1 && (
